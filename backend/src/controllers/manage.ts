...
export async function manage(ctx: Context): Promise<void> {
  ...
  const obj = await Game.findOneAndUpdate({'businesses.name': body.name}, {$set: {"businesses.$.managed": true}});

  const businessJson = JSON.parse(JSON.stringify(obj?.toObject().businesses.filter((elem: Business): boolean => elem.name === body.name)[0]));
