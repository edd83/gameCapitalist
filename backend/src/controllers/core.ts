...
export async function updateMoneyByBizName(name: string): Promise<void> {
  ...
  const businessJson = JSON.parse(JSON.stringify(firstDoc?.toObject().businesses.filter(
    (elem: Business): boolean => elem.name === name)[0]));
...
export async function buyManager(name: string, profit: number, cost: number, speed: number): Promise<number> {
  ...
  console.log(`Manager bought for ${cost}, money is ${obj?.toObject().money},
    profit is ${profit}, and will update money every ${speed / 1000}s`);
