...
export async function levelup(ctx: Context): Promise<void> {
  ...
  await Game.findOneAndUpdate({ 'businesses.name': body.name }, { $inc: { 'businesses.$.level': 1, 'money': -businessJson.profit },
    $mul: { 'businesses.$.profit': businessJson.multipleCoef } }, { new: true });

  console.log(`levelup to ${businessJson.level}`);
